...
import { InformationsOperateurComponent } from '../informations-operateur/informations-operateur.component';

@Component({
  selector: 'app-list-operateurs',
imports: [
    CommonModule,
    MatCardModule,
    MatTableModule,
  ...
  templateUrl: './list-operateurs.component.html',
  styleUrl: './list-operateurs.component.scss'
})
export class ListOperateursComponent implements OnInit {
  constructor(private service : AdminServiceService , private  router : Router ,
              private dialog: MatDialog ,private serviceSuperAdmin : SuperAdminService){}
displayedColumns: string[] = [
  ...
  dataSource: MatTableDataSource<Operateur>;
...
applyFilter(event: Event) {
  ...
  applyPlantFilter(selectedPlant: string[]) {
   ...
   applyPosteFilter(selectedPoste: string[]) {
    this.currentPosteFilter = selectedPoste;
    this.applyAllFilters();
  }

  applyMachineFilter(selectedMachine: string[]) {
    this.currentMachineFilter = selectedMachine;
    this.applyAllFilters();
  }
  ...
  applyAllFilters() {
    this.dataSource.filterPredicate = (data: Operateur, filter: string) => {
  ...
  data.machine?.toLowerCase().includes(this.searchFilter) ||
  data.poste?.toLowerCase().includes(this.searchFilter) ||
  ...
  this.currentPrenomFilter.includes(data.prenom);
    ...
    const posteMatch = this.currentPosteFilter.length === 0 || 
  this.currentPosteFilter.includes(data.poste);

    const machineMatch = this.currentMachineFilter.length === 0 || 
  this.currentMachineFilter.includes(data.machine);
      ...
      return searchMatch && matriculeMatch && segmentMatch && plantMatch && 
           prenomMatch && nomMatch && numeroTelephoneMatch && typeMatch && machineMatch && posteMatch;
    };
...
recupererListOperateurs() {
this.service.getAllOperateurs().subscribe({
    next: (data) => {
      this.operateurs = data;
      this.dataSource = new MatTableDataSource(this.operateurs);
...
updateAdmin(row: Admin) {
  this.router.navigate(['/ui-components/updateOperateur', row.matricule]);
}

viewAdmin(row: Admin) {
  this.service.getOperateur(row.matricule).subscribe({
    next: (operatorsData) => {
      const operatorsArray = Array.isArray(operatorsData) ? operatorsData : [operatorsData];

      this.dialog.open(InformationsOperateurComponent, {
        width: '500px',
        data: {
...
deleteAdmin(row: Admin) {
  ...
  Swal.fire({
    title: 'Confirmation',
    text: 'Êtes-vous sûr de vouloir supprimer cet opérateur ?',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      this.serviceSuperAdmin.deleteUser(+matricule).subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
            title: 'Supprimé !',
          ...
          this.recupererListOperateurs(); // Mise à jour du tableau
        },
        error: (err) => {
          Swal.fire({
            icon: 'error',
